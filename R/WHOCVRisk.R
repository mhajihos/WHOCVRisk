WHOCVRisk=function(region,age,gender,smoke,bmi,sbp)
{
  ...
  (region,age,gender,smoke,bmi,sbp)
  Var1=c("region","age2","gender","smoke","bmi2","sbp2")
  df=df[,Var1]

  Res=merge(df, Ref, by.x = c("region","age2","gender","smoke","bmi2","sbp2"),
            by.y = c("Region","Age","Gender","Smoke","BMI","SBP"),
            all.x = FALSE, all.y = TRUE)
  left_join(df,Ref,by=region)
